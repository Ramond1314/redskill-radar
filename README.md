...
小红书 REDSkill 排名看板：展示累计使用人数榜、7 天新增榜、当日新增榜和 SkillHub 公开条目，支持搜索、类别筛选、趋势卡片、封面 / 收藏筛选、排序、Skill 对比、详情抽屉、使用说明、预期效果、可复制调用示例和小红书分享卡片导出。已有公开案例的 Skill 使用小红书封面，缺少案例封面的 Skill 则根据自身风格生成独立定制封面。
...
- `skillHubList`：SkillHub 公开列表快照；页面分类映射为看板的视觉设计、内容增长、研究决策、产品开发、学习求职、生活效率和其他探索
- `skillhub-data.json`：SkillHub 条目快照；这些条目没有公开统一的使用人数指标，因此在 SkillHub 页签中显示为“—”，不会混入 REDSkill 热度榜
